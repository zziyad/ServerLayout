({
  ...
  method: async (payload) => {
    ...
    const schema = await lib.schemas.department.listSchema();
    ...
    try {
      const res = await domain.department.list(validatedData, context);
      return { status: 'fulfilled', response: res };
    } catch (err) {
      console.error('department/list error:', err);
      return {
        status: 'rejected',
        response: err.message || 'Failed to fetch departments',
      };
    }
  },
});
