...
- `file.read` / `file.write` authorize the **RPC**. Each file row is checked again in SQL (owner or grant). Hits the caller cannot read are not returned. Sidecar/FTS ranks ids only.
...
- Extracted text from pdf / docx / xlsx / txt / csv / json.
- Store **capped extracted text** (normalized) plus `tsvector`. Not the binary. Not a 200-char preview only.
- Binary without text: metadata only.
- Extract + index **async** after upload. Upload RPC must not wait on parse.
- Extractor caps (exact numbers in config at 5.4): max file bytes, max extracted chars, timeout. Over cap → `failed`, no hang.
- Index job identity: `hash + extractor_version`. Repeat `files/index` with the same pair does not duplicate rows or re-extract if status is `ready`.
- `index_status`: `pending` → `processing` → `ready` | `failed`. Search returns only `ready`.
...
Pagination (same as `user/list`): default **20**, max **100**. SQL `LIMIT` only. No unbounded list in JS.
...
- Columns include `index_status`, `extractor_version`, `hash`. Unique work key: `(hash, extractor_version)` or equivalent idempotent upsert.
