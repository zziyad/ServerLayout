...
class Static extends Place {
  ...
  getCacheHeaders(filePath) {
    if (filePath.endsWith('/index.html') || filePath === '/index.html') {
  ...
  async serve(url, transport, httpCode) {
    ...
    const absPath = node.path.join(this.application.path, url);
    if (absPath.startsWith(this.application.path)) {
      const stat = await node.fsp.stat(absPath).catch(() => null);
      if (!stat || !stat.isFile())
        return void transport.write(this.get('/index.html'), 200, 'html', {
          headers: this.getCacheHeaders('/index.html'),
        });
      const { size } = stat;
